    ...
    <hero-slider ref="slider1" :v-bind="props">
      <div v-for="i in cHero.images" :key="i.id">
        <div
          class="hero-image"
          :style="{
            backgroundImage: `linear-gradient(rgba(0, 0, 0, 0.5), rgba(0, 0, 0, 0.5)), url(${i.img})`,
          }"
        >
          <div class="hero-text">
            <h1 v-if="$i18n.locale !== 'de'">
              {{ i.text }}
            </h1>
            <div v-if="$i18n.locale === 'de'" class="hero-text--german">
              <h1>
                Sie sind auf der Suche nach einem Job im Ausland? <br />Unser
                Expertenteam bietet Ihnen:
              </h1>
              <ul>
                <li>Einen sicheren Arbeitsplatz</li>
                <li>Eine Unterkunft mit Wohlfühlatmosphäre</li>
                <li>Ein super Einkommen</li>
                <li>Verschiedenste Möglichkeiten der Weiterbildung</li>
              </ul>
            </div>
          </div>
        </div>
